...
const MORI_AUTH = {
    ...
    // ========== УЛУЧШЕНИЕ 10: ВОССТАНОВЛЕНИЕ ПАРОЛЯ ==========
    recoverPassword: async function(email) {
        if (!email || !email.includes('@')) {
            MORI_APP.showToast('❌ Введите корректный email', 'error');
            return false;
        }

        try {
            MORI_APP.showToast('🔄 Отправка инструкций...', 'info');
            
            const response = await MORI_API.recoverPassword?.({ email });
            
            if (response && response.success) {
                MORI_APP.showToast('📧 Инструкции отправлены на почту', 'success');
                this.showResetForm(email);
                return true;
            }
        } catch (error) {
            console.error('Ошибка восстановления:', error);
            MORI_APP.showToast('❌ Ошибка восстановления пароля', 'error');
        }
        return false;
    },

    showResetForm: function(email) {
        ...
        modal.innerHTML = `
            <div class="reset-content">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="reset-code" placeholder="XXXXXX" maxlength="6">
                </div>
                <div class="form-group">
                ...
                <button class="reset-btn" id="reset-submit">Сбросить пароль</button>
                <button class="reset-close">✕</button>
            </div>
        `;
        ...
        document.getElementById('reset-submit').addEventListener('click', async () => {
            ...
            if (!code || code.length !== 6) {
                MORI_APP.showToast('❌ Неверный код', 'error');
                return;
            }

            if (!newPass || newPass.length < 6) {
                MORI_APP.showToast('❌ Пароль должен быть минимум 6 символов', 'error');
                return;
            }

            if (newPass !== confirmPass) {
                MORI_APP.showToast('❌ Пароли не совпадают', 'error');
                return;
            }

            try {
                const response = await MORI_API.resetPassword?.({
                    email,
                    code,
                    newPassword: newPass
                });
                ...
                if (response && response.success) {
                    MORI_APP.showToast('✅ Пароль изменён', 'success');
                    modal.remove();
                }
            } catch (error) {
                MORI_APP.showToast('❌ Ошибка сброса пароля', 'error');
            }
        });

        modal.querySelector('.reset-close').addEventListener('click', () => {
            modal.remove();
        });
    },
